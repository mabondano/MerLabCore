...
/**
 * Igual a WithLogging2 pero con Plotly.js INLINE (sin CDN).
 */
public class MLPClassifyExample11_WithLogging3 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // frontera circular aproximada muestreando
        int grid = 100;
        ...
        // 5) Leer plotly.min.js inline
        String plotlyJs;
        try (InputStream in = MLPClassifyExample11_WithLogging3.class
                .getResourceAsStream("/web/plotly.min.js")) {
            if (in == null) throw new IOException("No se encontró plotly.min.js en /web/");
            plotlyJs = new String(in.readAllBytes(), StandardCharsets.UTF_8);
        }

        // 6) Montar HTML con la librería embebida
        String html = """
          <html>
           <head>
             <meta charset="utf-8"/>
           ...
           <body>
             ...
             <script>
               const trace0 = { x: %s, y: %s,
                 mode:'markers', marker:{color:'blue',size:6}, name:'Clase 0' };
               const trace1 = { x: %s, y: %s,
                 mode:'markers', marker:{color:'red', size:6}, name:'Clase 1' };
               const trace2 = { x: %s, y: %s,
                 mode:'markers', marker:{color:'rgba(0,0,0,0.2)',size:2}, name:'Frontera' };
        ...
        ModelInfo info = new ModelInfo.Builder("MLP Clasificador Circulos Inline")
    ...
    // —————— mismos métodos auxiliares que en WithLogging2 ——————
    ...
    private static double min(double[][] raw, int c) {
        double m = Double.POSITIVE_INFINITY; for (var r : raw) m = Math.min(m, r[c]); return m;
    }
    private static double max(double[][] raw, int c) {
        double m = Double.NEGATIVE_INFINITY; for (var r : raw) m = Math.max(m, r[c]); return m;
    }
    ...
    private static Layer initLayer(
        int neurons, int inputs, com.merlab.signals.nn.processor.ActivationFunction act
    ) {
